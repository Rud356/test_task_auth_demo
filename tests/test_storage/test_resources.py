...
async def test_role_deletion_effects_on_permission_for_resource(
    user_repo: UsersRepositorySQLA,
    roles_repo: RolesRepositorySQLA,
    resources_repo: ResourceRepositorySQLA,
    hashing_settings: HashingSettings,
    ...
    test_unused_role: Role = await roles_repo.create_role(
        CreateRoleRequest(role_name=f"demo_role {secrets.token_urlsafe(4)}")
    )
    await roles_repo.assign_role_to_user(demo_user.user_id, new_role.role_id)
    assert (await user_repo.get_user(demo_user.user_id)).roles == [new_role]
    ...
    assert await resources_repo.set_roles_permissions_on_resource(
        data.resource_id,
        ResourcePermissionsUpdate(
            role_id=test_unused_role.role_id,
            can_view_resource=True,
            can_edit_resource=True
        )
    )
    ...
    await roles_repo.delete_role(role_id=new_role.role_id)
    assert (await user_repo.get_user(demo_user.user_id)).roles == []

    assert len(
        await resources_repo.list_available_resources(demo_user.user_id)
    ) == 0
    accessible_resources: list[ResourceDetails] = await resources_repo.list_available_resources(
        new_registered_user.user_id
    )
    assert len(accessible_resources) == 1
    assert {
        role.role_id
        for role in accessible_resources[0].roles_permissions
        if any((role.can_edit_resource, role.can_view_resource))
    } == {test_unused_role.role_id}
